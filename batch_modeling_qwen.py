...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def chat(
        ...
        **kwargs,
        ...
        generation_config = generation_config if generation_config is not None else self.generation_config # 定义generation_config
        # generation_config.chat_format = "chatml" generation_config.max_length = 20
        ...
        if max_window_size is None:
            max_window_size = generation_config.max_window_size # 6144
        raw_text, context_tokens, audio_info = make_context( # raw_text为对应的原始文本，context_tokens为编码后的token，audio_info为对应的音频信息
            tokenizer,
            query,
            history=history,
            system=system,
            max_window_size=max_window_size, # max_window_size为上下文的限制长度，没什么用
            chat_format=generation_config.chat_format,
        )
        # raw_text: '<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\nAudio 1:<audio>/data2/guhao/data/asvspoof2019/LA/eval/wav/LA_E_4471223.wav</audio>\nCan you determine if this audio is fake or real?<|im_end|>\n<|im_start|>assistant\n'
        stop_words_ids.extend(get_stop_words_ids(
            generation_config.chat_format, tokenizer
        )) # stop_words_ids： [[151645], [151644]]
        input_ids = torch.tensor([context_tokens]).to(self.device) # 转化成tensor
        kwargs['audio_info'] = audio_info
        outputs = self.generate(
                    input_ids, # input_ids
                    stop_words_ids=stop_words_ids, # stop_words_ids
        ...
        response = decode_tokens(
            outputs[0],
            tokenizer,
            raw_text_len=len(raw_text), # '<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\nAudio 1:<audio>/data2/guhao/data/asvspoof2019/LA/eval/wav/LA_E_4471223.wav</audio>\nCan you determine if this audio is fake or real?<|im_end|>\n<|im_start|>assistant\n'
            context_length=len(context_tokens), # 97 为输入tensor长度
            chat_format=generation_config.chat_format,
            verbose=False,
            errors='replace',
            audio_info=audio_info
    ...
    def generate(
        ...
        **kwargs,
        ...
        if stop_words_ids is None:
            stop_words_ids = getattr(generation_config, "stop_words_ids", None) # 是none吗？
