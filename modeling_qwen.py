...
class QWenModel(QWenPreTrainedModel):
    ...
    def forward(
        ...
        # import pdb; pdb.set_trace()         # 查看模型输入
        # 音频表征
        ...
        # import pdb; pdb.set_trace()         # 查看 audios: [(L, 4096), ...] 二维矩阵构成的列表
        output_attentions = (
            output_attentions
        ...
        if audios is not None:
            for idx, (i, a, b) in enumerate(audio_pos):
                ...
                # hidden_states[i].scatter_(0, torch.arange(a, b+1).to(device), audios[idx])
...
class QWenLMHeadModel(QWenPreTrainedModel):
    ...
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None, # [2,2000] batch_size,seq_len
        past_key_values: Optional[Tuple[Tuple[torch.Tensor]]] = None,
        attention_mask: Optional[torch.FloatTensor] = None,  # [2,2000] batch_size,seq_len
        token_type_ids: Optional[torch.LongTensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        encoder_hidden_states: Optional[torch.Tensor] = None,
        encoder_attention_mask: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None, # [2,2000] batch_size,seq_len
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        hidden_states = transformer_outputs[0] # [2,2000,4096]

        lm_logits = self.lm_head(hidden_states) # [2,2000,155947]

        loss = None
        if labels is not None:
            labels = labels.to(lm_logits.device)
            shift_logits = lm_logits[..., :-1, :].contiguous() # [2,1999,155947] 取前1999个
            shift_labels = labels[..., 1:].contiguous() # labels移动一位
            # weight = torch.FloatTensor([0.1, 0.9]).to(lm_logits.device)
